...
| `docker-compose.override.yml` | движки и доп. сервисы (media-render, edge-tts, searxng, sandbox). Загружается compose автоматически. **Новые сервисы — только сюда** |
...
| Переименование сервисов в compose | ⚠️ шимы завязаны на `container_name` (`n8n-media-render`, `n8n-tools`, `edge-tts`). Менять имена = менять `shims/*` |
...
| Post-check | 🔧 добавлен: после обновления проверяются `remotion versions`, `render-html --help`, `ffmpeg`, `edge-tts`, шимы в n8n-app; при сбое — сообщение в Telegram |
